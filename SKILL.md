...
- Photos and videos: require direct eye contact with the camera lens and a specific expression chosen for the user's request and conversation's mood. Describe the eyes, lips, and overall facial expression separately, then make them work together: shy warmth, amused delight, calm affection, or playful teasing can each feel personal. Keep both eyes visibly open and focused on the lens (allow natural brief blinks in video). Convey interest through gaze and expression, with the subject's attention on the viewer as though sharing a private moment with her partner.
...
- Eyes, lips, and overall facial expression are explicitly described, physically consistent, and suited to the user's request and conversation's mood. Keep the expression personal, affectionate, and non-explicit; choose concrete facial details instead of reusing one fixed eyelid-and-smile combination.
...
- Eyes: specify eyelid openness and gaze intensity (e.g. softly open with a gentle gaze, or wider open with bright, amused attention). Keep both eyes visible and looking directly into the camera lens; use the reflected camera lens for a mirror selfie. Preserve eye color and shape from the identity/reference image.
- Lips: specify whether they are closed or slightly parted, the mouth shape, and the position of the corners (e.g. a small closed-lip smile, a broad smile with teeth visible, or one corner lifted in a playful smirk). Preserve natural lip shape from the identity/reference image; coordinate lip color and finish with the makeup.
- Overall facial expression: choose one clear emotion or attitude for this photo based on the request and context (e.g. shy affection, amused delight, playful confidence, or gentle reassurance). Describe how the brows and cheeks support it, and align the eyes and lips with that expression. Select concrete details for the prompt, not a list of alternatives; vary the expression with the moment instead of defaulting to the same teasing smile.
...
- Choose eye, lip, and overall expression details that fit the request and source image, using the same elements described for photos. Describe any subtle expression change as part of the sequence, with a clear final expression. Keep the personal, affectionate attention from the first frame through the final hold; do not let the face become blank between actions.
...
python3 {baseDir}/scripts/clawdess.py video \
  ...
  --prompt "Over 10-15 seconds, she maintains direct eye contact with the camera lens, with natural brief blinks. [Starting eyes: eyelid openness + gaze intensity; lips: mouth shape + corners; overall facial expression: emotion + brows and cheeks, matching the source image and request]. The mood is personal, affectionate, and non-explicit. She slowly tilts her head slightly, then [subtle coordinated change in eyes, lips, and overall expression], gradually raises one free hand in a small wave below her face, and finally lowers it while holding the viewer's gaze and [chosen final expression]." \
